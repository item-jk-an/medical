...
<table id="videoFlowTable"></table>
</body>
<script type="text/javascript">
    $(function (){

        $("#videoFlowTable").bootstrapTable({
            url:"<%=request.getContextPath()%>/flow/queryVideoFlow.do",
            method:"post",
            ...
            columns:[
                {field:'videoId',title:'',width:50,class:"danger",
                    formatter:function(value,row,index){   //  格式化  当前单元格内容
                        return "<input type='checkbox' value="+value+" name='chk'/>";
                    }
                },
                {field:'videoId',title:'视频ID', class:'warning',width:100},
                {field:'videoName',title:'视频名称', class:'success',width:100},
                {field:'videoId',title:'查看视频详情', class:'success',width:100,
                    formatter:function(value,row,index){   //  格式化  当前单元格内容
                        return "<input type='button' value='查看' onclick='chakan("+value+")'>"
                    }},
                {field:'videoId',title:'审批', class:'success',width:100,
                    formatter:function(value,row,index){   //  格式化  当前单元格内容
...
<script type="text/javascript">
        ...
        if($("[name='chk']:checked").length>0){
            ...
            $.ajax({
                ...
                success:function(){
                    bootbox.alert('删除成功');
                    //刷新
                    $("#videoFlowTable").bootstrapTable('refresh');
                }
    ...
    function chakan(id){
        dialog("<%=request.getContextPath()%>/flow/queryVideoFlowById.do?videoId="+id,"查看详情");
    }

    function yes(id){
        $.ajax({
            url:"<%=request.getContextPath()%>/flow/updateflowVideoShenById.do",
            type:"get",
            data:{"id":id},
            success:function(){
                bootbox.alert('审核通过');
    ...
    function no(id){
        $.ajax({
            url:"<%=request.getContextPath()%>/flow/updateflowVideoShenNOById.do",
            type:"get",
            data:{"id":id},
            success:function(){
