...
<table id="kitFlowTable"></table>
</body>
<script type="text/javascript">
    $(function (){

        $("#kitFlowTable").bootstrapTable({
            url:"<%=request.getContextPath()%>/flow/queryKitFlow.do",
            method:"post",
            ...
            columns:[
                {field:'kitId',title:'',width:50,class:"danger",
                    formatter:function(value,row,index){   //  格式化  当前单元格内容
                        return "<input type='checkbox' value="+value+" name='chk'/>";
                    }
                },
                {field:'kitId',title:'器械ID', class:'warning',width:100},
                {field:'kitName',title:'器械名称', class:'success',width:100},
                {field:'kitId',title:'查看器械详情', class:'success',width:100,
                    formatter:function(value,row,index){   //  格式化  当前单元格内容
                        return "<input type='button' value='查看' onclick='chakan("+value+")'>"
                    }},
                {field:'kitId',title:'审批', class:'success',width:100,
                    formatter:function(value,row,index){   //  格式化  当前单元格内容
...
<script type="text/javascript">
        ...
        if($("[name='chk']:checked").length>0){
            ...
            $.ajax({
                ...
                success:function(){
                    bootbox.alert('删除成功');
                    //刷新
                    $("#kitFlowTable").bootstrapTable('refresh');
                }
    ...
    function chakan(id){
        dialog("<%=request.getContextPath()%>/flow/queryKitFlowById.do?kitId="+id,"查看详情");
    }

    function yes(id){
        $.ajax({
            url:"<%=request.getContextPath()%>/flow/updateflowKitShenById.do",
            type:"get",
            data:{"id":id},
            success:function(){
                bootbox.alert('审核通过');
    ...
    function no(id){
        $.ajax({
            url:"<%=request.getContextPath()%>/flow/updateflowKitShenNOById.do",
            type:"get",
            data:{"id":id},
            success:function(){
