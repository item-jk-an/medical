...
<table id="infoFlowTable"></table>
</body>
<script type="text/javascript">
    $(function (){

        $("#infoFlowTable").bootstrapTable({
            url:"<%=request.getContextPath()%>/flow/queryInfo2Flow.do",
            method:"post",
            ...
            columns:[
                {field:'infoId',title:'',width:50,class:"danger",
                    formatter:function(value,row,index){   //  格式化  当前单元格内容
                        return "<input type='checkbox' value="+value+" name='chk'/>";
                    }
                },
                {field:'infoId',title:'info2ID', class:'warning',width:100},
                {field:'infoName',title:'info2名称', class:'success',width:100},
                {field:'infoId',title:'查看info2详情', class:'success',width:100,
                    formatter:function(value,row,index){   //  格式化  当前单元格内容
                        return "<input type='button' value='查看' onclick='chakan("+value+")'>"
                    }},
                {field:'infoId',title:'审批', class:'success',width:100,
                    formatter:function(value,row,index){   //  格式化  当前单元格内容
...
<script type="text/javascript">
        ...
        if($("[name='chk']:checked").length>0){
            ...
            $.ajax({
                ...
                success:function(){
                    bootbox.alert('删除成功');
                    //刷新
                    $("#infoFlowTable").bootstrapTable('refresh');
                }
    ...
    function chakan(id){
        dialog("<%=request.getContextPath()%>/flow/queryInfo2FlowById.do?info2Id="+id,"查看详情");
    }

    function yes(id){
        $.ajax({
            url:"<%=request.getContextPath()%>/flow/updateflowInfo2ShenById.do",
            type:"get",
            data:{"id":id},
            success:function(){
                bootbox.alert('审核通过');
    ...
    function no(id){
        $.ajax({
            url:"<%=request.getContextPath()%>/flow/updateflowInfo2ShenNOById.do",
            type:"get",
            data:{"id":id},
            success:function(){
